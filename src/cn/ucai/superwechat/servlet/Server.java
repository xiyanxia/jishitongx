...
//		java.util.List<GroupAvatar> list1 = biz.findPublicGroups(userName, pageId, pageSize);
		Result result = biz.findPublicGroups(userName, pageId, pageSize);
		JsonUtil.writeJsonToClient(result, response);
//		GroupAvatar[] arr=list1.toArray(new GroupAvatar[0]);
//		JsonUtil.writeJsonToClient(result, response);
//		Object[] d = new Object[3];
//		d[0] = result.isRetMsg();
//		System.out.println("d[0]="+d[0]);
//		d[1] = (Pager)result.getRetData();
//		System.out.println("d[0]="+d[1]);
//		d[2] = ((Pager)result.getRetData()).getPageData().toArray(new GroupAvatar[0]);
//		System.out.println("d[0]="+d[2]);
//		Gson gson = new Gson();
////		java.util.List<GroupAvatar> list = (java.util.List<GroupAvatar>) pager.getPageData();
...
//		}	
	...
	private void updateGroupName(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			groupNewName = new String(groupNewName.getBytes(I.ISON8859_1), I.UTF_8);
	...
	private void updateUserNickByName(HttpServletRequest request, HttpServletResponse response) {
		...
		String userNick = request.getParameter(I.User.NICK);
		try {
			userNick = new String(userNick.getBytes(I.ISON8859_1), I.UTF_8);
		} catch (UnsupportedEncodingException e1) {
			// TODO Auto-generated catch block
			e1.printStackTrace();
		}
	...
	private void createGroup(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			desc = new String(desc.getBytes(I.ISON8859_1), I.UTF_8);
			System.out.println("desc="+desc);
		} catch (UnsupportedEncodingException e1) {
			// TODO Auto-generated catch block
			e1.printStackTrace();
